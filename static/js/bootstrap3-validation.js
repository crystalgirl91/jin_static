   ...
   $.fn.validation.defaults = {
    ...
    var validateField = function(field, valid) { 
        ...
        if(form){
            ...
            if(fstyle == 0){
                controlGroup.find("#valierr").html(errorMsg);
    ...
    var validationForm = function(obj) {
        ...
        $(obj).find('input, textarea').each(function(){
            ...
            el.on('blur',function(){ // 失去焦点时
                if(el.attr('check-no-blur')!=undefined){
                    return
                }
                    ...
                    if (valid){
                        if ($.inArray('required',valid)>=0){
                            if (el.parent().hasClass('input-group')){
                                el.parent().parent().after('<span class="help-block" id="valierr" style="color:#FF9966">*</span>');
                            }
                            else
                            {
